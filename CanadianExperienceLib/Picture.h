...
private:
    ...
    Timeline mTimeline;
    /// Machine 1
    std::shared_ptr<MachineToCanadianAdapter> mMachine;

    /// Machine 2
    std::shared_ptr<MachineToCanadianAdapter> mMachine2;
...
/**
 * Sets the machine using a shared pointer to a MachineToCanadianAdapter.
 * @param machine A shared pointer to the MachineToCanadianAdapter.
 */
    void SetMachine(std::shared_ptr<MachineToCanadianAdapter> machine);

/**
 * Selects a machine for operation (version 2).
 * @param parent A pointer to the parent window.
 * @param num The machine number to select.
 */
    void SelectMachine2(wxWindow *parent, int num);

/**
 * Sets the machine using a shared pointer to a MachineToCanadianAdapter (version 2).
 * @param machine A shared pointer to the MachineToCanadianAdapter.
 */
    void SetMachine2(std::shared_ptr<MachineToCanadianAdapter> machine);

/**
 * Retrieves the machine 2.
 * @return A shared pointer to the MachineToCanadianAdapter.
 */
    std::shared_ptr<MachineToCanadianAdapter> GetMachine2();

/**
 * Retrieves the machine 1
 * @return A shared pointer to the MachineToCanadianAdapter.
 */
    std::shared_ptr<MachineToCanadianAdapter> GetMachine1();
